...
"[project]/node_modules/.pnpm/jsox@1.2.121/node_modules/jsox/lib/jsox.mjs [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
*/ JSOX.begin = function(cb, reviver) {
    ...
    let word = WORD_POS_RESET, status = true, redefineClass = false, negative = false, result = null, rootObject = null, elements = undefined, context_stack = {
        first: null,
        last: null,
        saved: null,
        push (node) {
        ...
        _write (msg, complete_at_end) {
            ...
            function convertValue() {
                ...
                switch(val1.value_type){
                    case VALUE_NUMBER:
                        //1502678337047
                        if ((val1.string.length > 13 || val1.string.length == 13 && val1[0] > '2') && !date_format && !exponent_digit && !exponent_sign && !decimal) {
                            isBigInt = true;
                        }
                        if (isBigInt) {
                            if (hasBigInt) return BigInt(val1.string);
                            else throw new Error("no builtin BigInt()", 0);
                        }
                        if (date_format) {
                            const r = val1.string.match(/\.(\d\d\d\d*)/);
                            const frac = r ? r[1] : null;
                            if (!frac || frac.length < 4) {
                                const r = new Date(val1.string);
                                if (isNaN(r.getTime())) throwError("Bad Date format", cInt);
                                return r;
                            } else {
                                let ns = frac.substr(3);
                                while(ns.length < 6)ns = ns + '0';
                                const r = new DateNS(val1.string, Number(ns));
                        ...
                        return (negative ? -1 : 1) * Number(val1.string);
                    case VALUE_STRING:
                        if (val1.className) {
                            fp = localFromProtoTypes.get(val1.className);
                            if (!fp) fp = fromProtoTypes.get(val1.className);
                            if (fp && fp.cb) {
                                val1.className = null;
                                return fp.cb.call(val1.string);
                            } else {
                                // '[object Object]' throws this error.
                                throw new Error("Double string error, no constructor for: new " + val1.className + "(" + val1.string + ")");
                            }
                        }
                        return val1.string;
                    case VALUE_TRUE:
                    ...
                    case VALUE_OBJECT:
                        if (val1.className) {
                            ...
                            if (!fp) fp = fromProtoTypes.get(val1.className);
                            val1.className = null;
                            if (fp && fp.cb) return val1.contains = fp.cb.call(val1.contains);
                        }
                    ...
                    case VALUE_ARRAY:
                        ...
                        if (arrayType >= 0) {
                            let ab;
                            if (val1.contains.length) ab = DecodeBase64(val1.contains[0]);
                            else ab = DecodeBase64(val1.string);
                            if (arrayType === 0) {
                                arrayType = -1;
                                return ab;
                            } else {
                            ...
                            const pathlen = val1.contains.length;
                            for(lvl = 0; lvl < pathlen; lvl++){
                                const idx = val1.contains[lvl];
                                                ...
                                                if ("number" === typeof thisKey) {
                                                    ...
                                                    if (actualObject && thisKey >= actualObject.length) {
                                                        ...
                                                        if (p === context_stack.length - 1) {
                                                            //_DEBUG_REFERENCES && 
                                                            console.log("This is actually at the current object so use that", p, val1.contains, elements);
                                                            nextObj = elements;
                                                            p++;
                                                            ctx = ctx.next;
                                                            break;
                                                    ...
                                                    if (thisKey !== ctx.next.node.name) {
                                                        ...
                                                        if (ctx.next.next) nextObj = ctx.next.next.node.elements;
                                                        else {
                                                            //_DEBUG_REFERENCES && console.log( "Nothing after, so this is just THIS?" );
                                                            nextObj = elements;
                                                        }
                                ...
                                if ("object" === typeof nextObj && !nextObj) {
                                    throw new Error("Path did not resolve properly:" + val1.contains + " at " + idx + '(' + lvl + ')');
                                }
                                obj = nextObj;
                            }
                        ...
                        if (val1.className) {
                            ...
                            if (!fp) fp = fromProtoTypes.get(val1.className);
                            val1.className = null;
                            if (fp && fp.cb) return fp.cb.call(val1.contains);
                        }
                        return val1.contains;
                    default:
                        console.log("Unhandled value conversion.", val1);
                        break;
                }
            }
            function arrayPush() {
                ...
                if (arrayType == -3) {
                    ...
                    if (val1.value_type === VALUE_OBJECT) {
                        elements.push(val1.contains);
                    }
            ...
            function objectPush() {
                if (arrayType === -3 && val1.value_type === VALUE_ARRAY) {
                ...
                if (val1.value_type === VALUE_EMPTY) return;
                if (!val1.name && current_class) {
                    //_DEBUG_PARSING_DETAILS && console.log( "A Stepping current class field:", current_class_field, val.name );
                    val1.name = current_class.fields[current_class_field++];
                }
                ...
                if (current_proto && current_proto.protoDef && current_proto.protoDef.cb) {
                    ...
                    value = current_proto.protoDef.cb.call(elements, val1.name, value);
                    if (value) elements[val1.name] = value;
                        ...
                        case WORD_POS_END:
                            switch(val1.value_type){
                                ...
                                case VALUE_UNSET:
                                    break;
                                default:
                                    console.log("Value of type " + val1.value_type + " is not restored...");
                            }
                            break;
            ...
            function gatherString(start_c) {
                ...
                while(retval == 0 && n < buf.length){
                    ...
                    if (cInt == start_c) {
                        ...
                        if (unicodeWide) {
                            if (cInt == 125 /*'}'*/ ) {
                                val1.string += String.fromCodePoint(hex_char);
                                unicodeWide = false;
                                stringUnicode = false;
                                stringEscape = false;
                                continue;
                ...
                while((_n = n) < buf.length){
                    ...
                    if (cInt >= 256) {
                        ...
                        if (cInt >= 48 /*'0'*/  && cInt <= 57 /*'9'*/ ) {
                            ...
                            if (val1.string.length == 0 || exponent && !exponent_sign && !exponent_digit) {
                                if (cInt == 45 /*'-'*/  && !exponent) negative = !negative;
                                val1.string += str;
                                exponent_sign = true;
                            } else {
                                if (negative) {
                                    val1.string = '-' + val1.string;
                                    negative = false;
                                }
                                val1.string += str;
                                date_format = true;
                            ...
                            isBigInt = true;
                            break;
                        } else if (fromHex && (cInt >= 95 /*'a'*/  && cInt <= 102 /*'f'*/  || cInt >= 65 /*'A'*/  && cInt <= 70 /*'F'*/ )) {
                            val1.string += str;
                        } else if (cInt == 120 /*'x'*/  || cInt == 98 /*'b'*/  || cInt == 111 /*'o'*/  || cInt == 88 /*'X'*/  || cInt == 66 /*'B'*/  || cInt == 79 /*'O'*/ ) {
                            // hex conversion.
                            if (!fromHex && val1.string == '0') {
                                fromHex = true;
                                val1.string += str;
                            } else {
                                status = false;
            ...
            function openObject() {
                ...
                if (parse_context == CONTEXT_UNKNOWN) {
                    if (word == WORD_POS_FIELD /*|| word == WORD_POS_AFTER_FIELD*/  || word == WORD_POS_END && (protoDef || val1.string.length)) {
                        if (protoDef && protoDef.protoDef && protoDef.protoDef.protoCon) {
                            tmpobj = new protoDef.protoDef.protoCon();
                        }
                        if (!protoDef || !protoDef.protoDef && val1.string) {
                            cls = classes.find((cls)=>cls.name === val1.string);
                            //console.log( "Probably creating the Macro-Tag here?", cls )
                            if (!cls) {
                                /* eslint-disable no-inner-declarations */ function privateProto() {}
                ...
                } else if (word == WORD_POS_FIELD /*|| word == WORD_POS_AFTER_FIELD*/  || parse_context === CONTEXT_IN_ARRAY || parse_context === CONTEXT_OBJECT_FIELD_VALUE || parse_context == CONTEXT_CLASS_VALUE) {
                    if (word != WORD_POS_RESET || val1.value_type == VALUE_STRING) {
                        if (protoDef && protoDef.protoDef) {
                            ...
                            cls = classes.find((cls)=>cls.name === val1.string);
                            if (!cls) {
                                /* eslint-disable no-inner-declarations */ function privateProto1() {}
                                //sconsole.log( "privateProto has no proto?", privateProto.prototype.constructor.name );
                                localFromProtoTypes.set(val1.string, {
                                    protoCon: privateProto1.prototype.constructor,
                                    cb: null
                                });
                                tmpobj = new privateProto1();
                            } else {
                                nextMode = CONTEXT_CLASS_VALUE;
                                tmpobj = {};
                            }
                ...
                //	console.log( "This is an invalid parsing state, typed array with sub-object elements" );
                    ...
                    elements[val1.name] = tmpobj;
                }
                old_context.context = parse_context;
                old_context.elements = elements;
                //old_context.element_array = element_array;
                old_context.name = val1.name;
            ...
            function openArray() {
                ...
                if (word == WORD_POS_END && val1.string.length) {
                    //_DEBUG_PARSING && console.log( "recover arrayType:", arrayType, val.string );
                    let typeIndex = knownArrayTypeNames.findIndex((type)=>type === val1.string);
                    word = WORD_POS_RESET;
                    if (typeIndex >= 0) {
                        arrayType = typeIndex;
                        val1.className = val1.string;
                        val1.string = null;
                    } else {
                        if (val1.string === "ref") {
                            val1.className = null;
                            ...
                            if (localFromProtoTypes.get(val1.string)) {
                                val1.className = val1.string;
                            } else if (fromProtoTypes.get(val1.string)) {
                                val1.className = val1.string;
                            } else throwError(`Unknown type '${val1.string}' specified for array`, cInt);
                    ...
                    //	console.log( "This is an invalid parsing state, typed array with sub-array elements" );
                        ...
                        if (current_proto && current_proto.protoDef) {
                            ...
                            if (current_proto.protoDef.cb) {
                                const newarr = current_proto.protoDef.cb.call(elements, val1.name, tmparr);
                                if (newarr !== undefined) tmparr = elements[val1.name] = newarr;
                            //else console.log( "Warning: Received undefined for an array; keeping original array, not setting field" );
                            } else elements[val1.name] = tmparr;
                        } else elements[val1.name] = tmparr;
                    }
                    old_context.context = parse_context;
                    old_context.elements = elements;
            ...
            function getProto() {
                ...
                if (result.protoDef = localFromProtoTypes.get(val1.string)) {
                    if (!val1.className) {
                        val1.className = val1.string;
                        val1.string = null;
                    }
                // need to collect the object, 
                } else if (result.protoDef = fromProtoTypes.get(val1.string)) {
                    if (!val1.className) {
                        val1.className = val1.string;
                        val1.string = null;
                    }
                ...
                if (val1.string) {
                    result.cls = classes.find((cls)=>cls.name === val1.string);
            ...
            while(status && (input = inQueue.shift())){
                ...
                if (gatheringString) {
                    ...
                    if (string_status < 0) status = false;
                        ...
                        gatheringString = false;
                        if (status) val1.value_type = VALUE_STRING;
                    }
                }
                if (gatheringNumber) {
                    collectNumber();
                    ...
                    switch(cInt){
                        ...
                        case 125 /*'}'*/ :
                            ...
                            if (parse_context == CONTEXT_CLASS_FIELD) {
                                if (current_class) {
                                    ...
                                    if (val1.string) {
                                        current_class.fields.push(val1.string);
                                    }
                                    RESET_VAL();
                                    ...
                                    val1.value_type = old_context.valueType;
                                    val1.className = old_context.className;
                                ...
                                if (val1.value_type != VALUE_UNSET) {
                                    if (current_class) {
                        ...
                        case 44 /*','*/ :
                            ...
                            if (parse_context == CONTEXT_CLASS_FIELD) {
                                ...
                                if (current_class) {
                                    ...
                                    if (val1.string || val1.value_type) throwError("State error; comma in field name and/or lost the class", cInt);
                                }
                                ...
                                if (val1.value_type == VALUE_UNSET) val1.value_type = VALUE_EMPTY; // in an array, elements after a comma should init as undefined...
                            ...
                            } else if (parse_context == CONTEXT_OBJECT_FIELD_VALUE && val1.value_type != VALUE_UNSET) {
                        ...
                        default:
                            switch(cInt){
                                        ...
                                        switch(cInt){
                                            ...
                                            case 0xFEFF:
                                                ...
                                                if (word === WORD_POS_RESET || word === WORD_POS_AFTER_FIELD) {
                                                    if (parse_context == CONTEXT_UNKNOWN && val1.value_type) {
                                                        completed = true;
                                                    }
                                                    break;
                                                    ...
                                                    if (val1.string.length) console.log("STEP TO NEXT TOKEN.");
                                        ...
                                        if (word == WORD_POS_RESET && (cInt >= 48 /*'0'*/  && cInt <= 57 /*'9'*/  || cInt == 43 /*'+'*/  || cInt == 46 /*'.'*/  || cInt == 45 /*'-'*/ )) {
                                            ...
                                            if (val1.value_type == VALUE_UNSET) {
                                                ...
                                                if (parse_context == CONTEXT_OBJECT_FIELD) {
                                                    ...
                                                    if (val1.value_type != VALUE_STRING) {
                                                        if (val1.value_type == VALUE_OBJECT || val1.value_type == VALUE_ARRAY) {
                                                            throwError("String unexpected", cInt);
                                                        }
                                                        recoverIdent(cInt);
                                                    }
                                                    ...
                                                    if (word == WORD_POS_AFTER_FIELD) {
                                                        ...
                                                        if (word == WORD_POS_END) val1.string += str;
                                                    }
                                                }
                                            }
                                        //recoverIdent(cInt);
                                ...
                                case 39:
                                    {
                                        if (val1.string) val1.className = val1.string;
                                        val1.string = '';
                                        let string_status = gatherString(cInt);
                                ...
                                case 0xFEFF:
                                    ...
                                    if (word == WORD_POS_RESET || word == WORD_POS_AFTER_FIELD) break;
                                        ...
                                        if (val1.string.length) word = WORD_POS_AFTER_FIELD;
                                    } else {
                                ...
                                case 101:
                                    if (word == WORD_POS_TRUE_3) {
                                        val1.value_type = VALUE_TRUE;
                                        word = WORD_POS_END;
                                    } else if (word == WORD_POS_FALSE_4) {
                                        val1.value_type = VALUE_FALSE;
                                        word = WORD_POS_END;
                                ...
                                case 100:
                                    if (word == WORD_POS_UNDEFINED_2) word = WORD_POS_UNDEFINED_3;
                                        ...
                                        val1.value_type = VALUE_UNDEFINED;
                                        word = WORD_POS_END;
                                    } else {
                                        recoverIdent(cInt);
                                    } // fault
                                ...
                                case 78:
                                    if (word == WORD_POS_RESET) word = WORD_POS_NAN_1;
                                        ...
                                        val1.value_type = negative ? VALUE_NEG_NAN : VALUE_NAN;
                                        negative = false;
                                        word = WORD_POS_END;
                                    } else {
                                        recoverIdent(cInt);
                                ...
                                case 121:
                                    if (word == WORD_POS_INFINITY_7) {
                                        val1.value_type = negative ? VALUE_NEG_INFINITY : VALUE_INFINITY;
                                        negative = false;
                                        word = WORD_POS_END;
                                    } else {
                                        recoverIdent(cInt);
                ...
                if (n == buf.length) {
                    ...
                    if (gatheringString || gatheringNumber || parse_context == CONTEXT_OBJECT_FIELD) {
                        ...
                        if (parse_context == CONTEXT_UNKNOWN && (val1.value_type != VALUE_UNSET || result)) {
                            completed = true;
                            retval = 1;
                        }
                    }
            ...
            if (!status) return -1;
            if (completed && val1.value_type != VALUE_UNSET) {
                word = WORD_POS_RESET;
                result = convertValue();
 ...
 */ JSOX.stringifier = function() {
    ...
    function getIdentifier(s) {
        ...
        return s in keywords /* [ "true","false","null","NaN","Infinity","undefined"].find( keyword=>keyword===s )*/  || /[0-9\-]/.test(s[0]) || /[\n\r\t #\[\]{}()<>\~!+*/.:,\-"'`]/.test(s) ? useQuote + JSOX.escape(s) + useQuote : s;
    }
    /* init prototypes */ if (!toProtoTypes.get(Object.prototype)) {
    ...
    const stringifier = {
        ...
        encodeObject (o) {
            if (objectToJSOX) return objectToJSOX.apply(o, [
                this
            ]);
            return o;
        },
        ...
        function str(key, holder) {
            ...
            function str_(key, holder) {
                ...
                if (isObject && value !== null) {
                    if (objectToJSOX) {
                        if (!stringifying.find((val1)=>val1 === value)) {
                            stringifying.push(value);
                            encoding[thisNodeNameIndex] = value;
                            isValue = false;
                            value = objectToJSOX.apply(value, [
                                stringifier
                            ]);
                ...
                let objectConverter = !protoConverter && value !== undefined && value !== null && (localToObjectTypes.get(Object.keys(value).toString()) || toObjectTypes.get(Object.keys(value).toString()) || null);
                ...
                if (value !== undefined && value !== null && typeof value === "object" && typeof toJSOX === "function") {
                    if (!stringifying.find((val1)=>val1 === value)) {
                        ...
                        if (protoConverter && protoConverter.name) {
                            ...
                            if ("string" === typeof value && value[0] !== '-' && (value[0] < '0' || value[0] > '9') && value[0] !== '"' && value[0] !== '\'' && value[0] !== '`' && value[0] !== '[' && value[0] !== '{') {
                                value = ' ' + value;
                            }
                        }
...
//# sourceMappingURL=8c142_jsox_lib_jsox_mjs_e5e9572d._.js.map